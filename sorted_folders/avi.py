...
class DBQuery(DBCommon):
    ...
    ##################################
    def _spec_from_index_without_office(self):
        spec={"nested":[],"types":{}}
        r=self._request(self._requests.get,self._host+"/"+self.temp_index+"/_mapping",params=self._include_type_name)
        for index1 in r: 
            self._spec_from_mapping(spec,"",r[index1]["mappings"]["properties"])
        return spec
    #########################################
    ...
    def search_with_ascTimeSort(self, queries, size=10000, spec=None):
        if spec is None: spec=self._spec_from_index()
        dsl=compile(queries,spec)
        try:
            r=self._request(self._requests.post,self._host+"/"+self._index+"/_search",json={"sort":[{"time":{"order": "asc"}}],"query":dsl,"size":size, "seq_no_primary_term": True})
    ...
    #########################################
    def search_without_size(self, queries, spec=None):
        if spec is None: spec=self._spec_from_index()
        dsl=compile(queries,spec)
        r=self._request(self._requests.post,self._host+"/"+self._index+"/_search",json={"query":dsl, "seq_no_primary_term": True})
        for x in r["hits"]["hits"]:
            yield x
...
result1 = add_num(50)
result2 = add_num(5, 29)

print(result1, result2)

    def search_without_office(self, queries, size=10000, spec=None):
        if spec is None: spec=self._spec_from_index_without_office()
        dsl=compile(queries,spec)
        r=self._request(self._requests.post,self._host+"/"+self.temp_index+"/_search",json={"query":dsl,"size":size, "seq_no_primary_term": True})
        for x in r["hits"]["hits"]:
            yield x
    #########################################
    ...
    def delete_bulk(self, bulk, batch=500, refresh="false"):
    ...
    def update(self, _id, info, seq_no=None, primary_term=None):
        ...
        return self._request(self._requests.post,self._host+"/"+self._index+"/_doc/"+_id+"/_update?retry_on_conflict=5",params=options,json={"doc":info})  #ES6.8
    ...
    def update_bulk(self, updates, batch=500):
        ...
        while updates:
            ...
            for u in updates[0:batch]:
                ...
                cmds.append({ "doc": u[1] ,"doc_as_upsert" : True})
